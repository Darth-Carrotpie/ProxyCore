...
public class UnlockablesSampleDriver : MonoBehaviour {
    ...
    private void Scenario5_UnlockedCharacters() {
        ...
        sb.Append($"{unlocked.Count} unlocked: ");
    ...
    private void Scenario8_Standalone() {
        ...
        if (before)
            UnlockManager.Lock(_standaloneItem);
        else
            UnlockManager.Unlock(_standaloneItem);
        bool after = UnlockManager.IsUnlocked(_standaloneItem);
