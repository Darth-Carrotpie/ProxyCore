...
namespace ProxyCore {
    [ExecuteInEditMode]
    public class EventCoordinatorNew : BaseRegistry<EventDefinition> {
...
#if UNITY_EDITOR
        [NameList(typeof(ProxyCore.Editor.PropertyDrawersHelper), "AllEventNames")]
#endif
            ...
            if (Instance.attachmentsDictionary.TryGetValue(eventName, out thisEvent)) {
                if (Instance.showAttachedEvents && Instance.enableDebugging) {
                    if (!Instance.ignoreAttachedEvents.list.Contains(eventName))
                        Debug.LogWarning("<Has Attachment> M:" + eventName + ": " + message);
                }
                thisEvent.Invoke(message);
                /*             try {
                                thisEvent.Invoke(message);
        ...
        public static bool HasEvent(string eventName, UnityAction<GameMessage> listener) {
            ...
            if (Instance.eventDictionary.TryGetValue(eventName, out thisEvent)) {
                ...
                for (int i = 0; i < thisEvent.GetPersistentEventCount(); i++) {
                    Debug.Log(thisEvent.GetPersistentMethodName(i) + " :eventMethodName. evnt_mng_lsn:" + listener.GetType().Name);
