...
namespace ProxyCore.Editor
{
    ...
    public class EventMessageCodeGenerator : AssetPostprocessor
    {
        ...
        {
            // Register first, generate second. Accessors bake an event's ID into
            // GetDefinition(<id>), and GetDefinition reads the registry's serialized
            // definitions list — an event that is not in that list resolves to null at
            // runtime even when the baked ID matches the asset exactly. The auto-refresh
            // postprocessor normally gets here first, but both are debounced independently,
            // so do it here too rather than depend on which delayCall wins.
        ...
        private static string GenerateTriggerEventClass(List<(EventMessage evt, string assetPath)> events)
        {
            ...
            sb.AppendLine("// Regenerate via ProxyCore > Regenerate Event Accessors");
            sb.AppendLine();
